...
prepare-fs:
	@echo -e "\033[0;33m>>> >>> >>> >>> >>> >>> >>> >>> Prepare Filesystem\033[0m"
	@mkdir -p ./build
	@mkdir -p ./build/logs
...
prepare-wordpress:
	@echo -e "\033[0;33m>>> >>> >>> >>> >>> >>> >>> >>> Prepare Wordpress CMS before tests\033[0m"
...
test-joomla:
	@echo -e "\033[0;33m>>> >>> >>> >>> >>> >>> >>> >>> Run unit-tests for Joomla! CMS\033[0m"
	@php ./vendor/phpunit/phpunit/phpunit --configuration ./phpunit-joomla.xml.dist
	@echo ""

test-wordpress:
	@echo -e "\033[0;33m>>> >>> >>> >>> >>> >>> >>> >>> Run unit-tests for Wordpress\033[0m"
